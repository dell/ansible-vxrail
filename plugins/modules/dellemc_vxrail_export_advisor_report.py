...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_export_advisor_report

short_description: Export advisor report
...
EXAMPLES = r'''
    - name: Start to export advisor report
      dellemc_vxrail_export_advisor_report:
        vxmip: "{{ vxmip }}"
        vcadmin: "{{ vcadmin }}"
        vcpasswd: "{{ vcpasswd }}"
        format: "{{ format }}"
...
API = "/cvs/report"
MODULE = "dellemc_vxrail_export_advisor_report"
LOG_FILE_PATH = "/tmp/vxrail_ansible_export_advisor_report.log"
...
class VxRailCluster():
    def __init__(self):
        ...
        self.report_id = module.params.get('report_id')
    ...
    def get_versioned_response(self, api_instance, module_path):
        ...
        # Calls versioned method as attribute (ex: v1_cvs_report_key_format)
        call_string = self.api_version_string + '_cvs_report_key_format'
        # call_string = 'cvs_report_key_format'
        LOGGER.info("Using utility method: %s\n", call_string)
        api_cvs_report_key_format = getattr(api_instance, call_string)
        # get the parameters
        params = self.getRequestParams()
        LOGGER.info(f"params: {params}")
        return api_cvs_report_key_format(self.report_id, self.format, **params)
    ...
    def export_advisor_report(self):
        # create an instance of the API class
        api_instance = vxrail_ansible_utility.CVSPublicApi(vxrail_ansible_utility.ApiClient(self.configuration))
        try:
            # export compliance report
            return self.get_versioned_response(api_instance, "Get /cvs/report/{key}/{format}")
        except ApiException as e:
            LOGGER.error("Exception when calling CVSPublicApi->%s_cvs_report_key_format: %s\n", self.api_version_string, e)
            return 'error'         
...
def main():
    ...
    LOGGER.info('----Start to export advisor report: ----')
    response = VxRailCluster().export_advisor_report()
    if response == "error":
        module.fail_json(
            msg=f"Call {API} API failed, please see log file {LOG_FILE_PATH} for more error details.")

    LOGGER.info(f"Http Result: Status code - {response.status}")
