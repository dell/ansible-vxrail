...
DOCUMENTATION = r"""
---
module: dellemc_vxrail_dell_identity_service_start_device_auth

short_description: Initiate a new device code flow
...
EXAMPLES = r"""
    - name: Start device authentication via Dell Identity Service
...
RETURN = r"""
user_code:
  description: user code. This is the code the user needs to enter after.
  returned: always
  type: str
verification_uri:
  description: verification_uri. URI for user authentication.
  returned: always
  type: str
verification_uri_complete_value:
  description: verification_uri_complete_value. This URL is the combination of verification URI and user code.
  returned: always
  type: str
expires_in:
  description: When the return verification URL and user code expire, the value is provided in seconds.
  returned: always
  type: int
"""
...
MODULE = "dellemc_vxrail_dell_identity_service_start_device_auth"
LOG_FILE_PATH = "/tmp/vxrail_ansible_dell_identity_service_start_device_auth.log"
...
class VxRailCluster:
    ...
    def get_versioned_response(self, api_instance, module_path):
        ...
        # Calls versioned method as attribute (ex: v1_post_start_device_auth)
        call_string = self.api_version_string + "_post_start_device_auth"
    ...
    def start_device_auth(self):
        ...
        try:
            return self.get_versioned_response(api_instance, "Post /device-auth/start")
        except ApiException as e:
            LOGGER.error(
                "Exception when calling Dell Identity ServiceApi->%s_post_start_device_auth: %s\n", self.api_version_string, e
            )
            return "error"
...
def main():
    ...
    LOGGER.info("---- Dell Identity Service start device authentication: ----")
    response = VxRailCluster().start_device_auth()
    if response == "error":
    ...
    module.exit_json(
        changed=True, message=f"Dell Identity Service start device authentication successfully, data: {response.data}"
    )
