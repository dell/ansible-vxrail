...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_cluster_layer3_get_segments
...
LOGGER = utils.get_logger(
    "dellemc_vxrail_cluster_layer3_get_segments", "/tmp/vxrail_ansible_layer3_get_segments.log",
...
class VxRailCluster():
    ...
    def get_versioned_response(self, api_instance, module_path):
        ...
        if self.api_version_number is None:
            ...
            self.api_version_string = utils.get_api_version_string(self.vxm_ip, self.api_version_number,
                                                                   module_path, LOGGER)
        call_string = self.api_version_string + '_cluster_layer3_segments_get'
        LOGGER.info("Using utility method: %s\n", call_string)
        api_cluster_layer3_segments_get = getattr(api_instance, call_string)
        return api_cluster_layer3_segments_get()
    ...
    def get_layer3_segments(self):
        ...
        try:
            ...
            return 'error'
        LOGGER.info("%s/cluster/layer3/segments api response: %s\n", self.api_version_string, response)
        data = response
        if not data:
...
def main():
    ...
    if result == 'error':
        module.fail_json(
            msg="API /cluster/layer3/segments get call failed, please refer /tmp/vxrail_ansible_layer3_get_segments.log")
    vx_facts = {'Cluster_Layer3_Get_Segments_Information': result}
    vx_facts_result = dict(changed=False, Cluster_Layer3_Segments_API=vx_facts)
