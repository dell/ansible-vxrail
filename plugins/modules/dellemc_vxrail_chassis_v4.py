...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_chassis_v4
...
description:
- This module will call v4/chassis api to get chassis list & every node information for each chassis and provide more host level information.
  If "chassis_id" is provided, this module will call v4/chassis/{chassis_id} to get information about the user-specified VxRail chassis.
options:
...
LOGGER = utils.get_logger(
    "dellemc_vxrail_chassis_v4", "/tmp/vxrail_ansible_chassis_v4.log", log_devel=logging.DEBUG)
...
class VxRailHosts():
    ...
    def get_v4_chassis(self):
        # create an instance of the API class
        api_instance = vxrail_ansible_utility.ChassisInformationApi(
            vxrail_ansible_utility.ApiClient(self.configuration))
        try:
            # get all v4 chassis information
            response = api_instance.v4_chassis_get()
        except ApiException as e:
            LOGGER.error(
                "Exception when calling ChassisInformationApi->v4_chassis_get: %s\n", e)
            return 'error'
        LOGGER.info("v4/chassis api response: %s\n", response)
        datalist = response
        if not datalist:
            return "No available hosts"
        return self._get_info_list(self._generate_chassis_info, datalist)

    def get_specific_v4_chassis(self):
        # create an instance of the API class
        api_instance = vxrail_ansible_utility.ChassisInformationApi(
            vxrail_ansible_utility.ApiClient(self.configuration))
        try:
            # get specific v4 chassis information by chassis id
            response = api_instance.v4_chassis_id_get(self.chassis_id)
        except ApiException as e:
            LOGGER.error(
                "Exception when calling ChassisInformationApi->v4/chassis/{chassis_id}: %s\n", e)
            return 'error'
        LOGGER.info("v4/chassis/{chassis_id} api response: %s\n", response)
        data = response
        if not data:
...
def main():
    ...
    result = VxRailHosts().get_v4_chassis()if (module.params.get('chassis_id')
                                               ) == "all" else VxRailHosts().get_specific_v4_chassis()
    if result == 'error':
        module.fail_json(
            msg="Call V4/chassis API failed,please see log file /tmp/vxrail_ansible_chassis_v4.log for more error details.")
    vx_facts = {"Chassis_Information": result}
    vx_facts_result = dict(changed=False, V4_Chassis_API=vx_facts)
