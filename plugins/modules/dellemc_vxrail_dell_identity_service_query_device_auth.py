...
DOCUMENTATION = r"""
---
module: dellemc_vxrail_dell_identity_service_query_device_auth

short_description: Get the existing support account configuration
...
EXAMPLES = r"""
    - name: Query device authentication via Dell Identity Service
...
RETURN = r"""
account_name:
  description: Support account name.
  returned: always
  type: str
token_expiration:
  description: Token expiration value. This is Unix epoch value.
  returned: always
  type: int
"""
...
API = "/device-auth/query"
MODULE = "dellemc_vxrail_dell_identity_service_query_device_auth"
LOG_FILE_PATH = "/tmp/vxrail_ansible_dell_identity_service_query_device_auth.log"
...
class VxRailCluster:
    ...
    def get_versioned_response(self, api_instance, module_path):
        ...
        # Calls versioned method as attribute (ex: v1_get_query_device_auth)
        call_string = self.api_version_string + "_get_query_device_auth"
    ...
    def query_device_auth(self):
        ...
        try:
            return self.get_versioned_response(api_instance, "Get /device-auth/query")
        except ApiException as e:
            LOGGER.error(
                "Exception when calling Dell Identity ServiceApi->%s_get_query_device_auth: %s\n", self.api_version_string, e
            )
            return "error"
...
def main():
    ...
    LOGGER.info("---- Dell Identity Service query device authentication: ----")
    response = VxRailCluster().query_device_auth()
    if response == "error":
    ...
    module.exit_json(
        changed=True, message=f"Dell Identity Service query device authentication successfully, data: {response.data}"
    )
