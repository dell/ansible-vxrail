...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_system_update_proxy

short_description: Update Existing VxRail System Proxy settings

description:
- This module will update existing the VxRail system's proxy settings. Returns the newly-set values.
options:
...
EXAMPLES = r'''
  - name: Updates existing the VxRail System proxy settings
...
API = "PATCH /system/proxy"
MODULE = "dellemc_vxrail_system_update_proxy"
LOG_FILE_PATH = "/tmp/vxrail_ansible_system_update_proxy.log"
...
class VxRailCluster():
    ...
    def get_versioned_response(self, api_instance, module_path, proxy_change_info):
        ...
        # Calls versioned method as attribute (ex: v1_system_proxy_patch)
        call_string = self.api_version_string + '_system_proxy_patch'
        LOGGER.info("Using utility method: %s\n", call_string)
        system_proxy_patch = getattr(api_instance, call_string)
        return system_proxy_patch(proxy_change_info)

    def patch_proxy(self):
        proxy_change_info = {}
        spec_info = {
            "server": self.server,
        ...
        try:
            ...
            response = self.get_versioned_response(api_instance, "PATCH /system/proxy", proxy_change_info)
        except ApiException as e:
            LOGGER.error("Exception when calling SystemProxySettingsApi->%s_system_proxy_patch: %s\n", self.api_version_string, e)
            return 'error'
        ...
        LOGGER.info("Updated proxy to: %s\n", proxy_change_info)
