...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_hosts_v5
...
description:
- This module will get information of all hosts in the cluster with v5/hosts api or one host specified by serial number wiht v5/hosts/{sn}.
options:
...
LOGGER = utils.get_logger(
    "dellemc_vxrail_hosts_v5", "/tmp/vxrail_ansible_hosts_v5.log", log_devel=logging.DEBUG)
...
class VxRailHosts():
    ...
    def get_v5_hosts(self):
        # create an instance of the API class
        api_instance = vxrail_ansible_utility.HostInformationApi(
            vxrail_ansible_utility.ApiClient(self.configuration))
        try:
            # get all v5 hosts information
            response = api_instance.v5_hosts_get()
        except ApiException as e:
            LOGGER.error(
                "Exception when calling HostInformationApi->v5_hosts_get: %s\n", e)
            return 'error'
        LOGGER.info("v5/hosts api response: %s\n", response)
        datalist = response
        if not datalist:
            return "No available hosts"
        return self._get_info_list(self._generate_host_info_from_response_data, datalist)

    def get_specific_v5_hosts(self):
        # create an instance of the API class
        api_instance = vxrail_ansible_utility.HostInformationApi(
            vxrail_ansible_utility.ApiClient(self.configuration))
        try:
            # get specific v5 host information by sn
            response = api_instance.v5_hosts_sn_get(self.host_sn)
        except ApiException as e:
            LOGGER.error(
                "Exception when calling HostInformationApi->v5_hosts_sn_get: %s\n", e)
            return 'error'
        LOGGER.info("v5/hosts/{sn} api response: %s\n", response)
        data = response
        if not data:
...
def main():
    ...
    result = VxRailHosts().get_v5_hosts() if (module.params.get('host_sn')
                                              ) == "all" else VxRailHosts().get_specific_v5_hosts()
    if result == 'error':
        module.fail_json(
            msg="Call V5/Hosts API failed,please see log file /tmp/vxrail_ansible_hosts_v5.log for more error details.")
    vx_facts = {"Hosts_Information": result}
    vx_facts_result = dict(changed=False, V5_Hosts_API=vx_facts)
