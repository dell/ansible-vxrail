...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_lcm_v1_precheck
...
LOG_FILE_NAME = "/tmp/vxrail_ansible_lcm_v1_precheck.log"
LOGGER = utils.get_logger("dellemc_vxrail_lcm_v1_precheck", LOG_FILE_NAME, log_devel=logging.DEBUG)
...
class VxRailLCMPRCHECK():
    ...
    def v1_lcm_precheck(self, lcm_precheck_json):
        ...
        try:
            # start LCM Precheck with v1 api
            response = api_instance.precheck_v1(request_body)
        except ApiException as e:
            LOGGER.error("Exception when calling LCMPreCheckApi->precheck_v1: %s\n", e)
            return 'error'
...
def main():
    ...
    request_body = lcm_precheck_json
    lcm_precheck_request_id = VxRailLCMPRCHECK().v1_lcm_precheck(request_body)
    LOGGER.info('LCM Precheck: VxRail task_ID: %s.', lcm_precheck_request_id)
    if lcm_precheck_request_id == "error":
        module.fail_json(
            msg="lcm precheck request id is not returned. Please see the /tmp/vxrail_ansible_lcm_v1_precheck.log for more details")
    ...
    if lcm_precheck_status == 'COMPLETED':
        ...
        vx_facts_result = dict(failed=True, LCM_V1_Precheck=vx_lcm_precheck,
                               msg="LCM Precheck has failed. Please see the /tmp/vxrail_ansible_lcm_v1_precheck.log for more details")
        module.exit_json(**vx_facts_result)
    vx_lcm_precheck = {'status': lcm_precheck_status, 'request_id': lcm_precheck_request_id}
    vx_facts_result = dict(changed=False, LCM_V1_Precheck=vx_lcm_precheck,
                           msg="LCM Precheck is successful. Please see the /tmp/vxrail_ansible_lcm_v1_precheck.log for more details")
    module.exit_json(**vx_facts_result)
