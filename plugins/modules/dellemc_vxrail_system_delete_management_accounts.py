...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_system_delete_management_accounts

short_description: Delete a vCenterManagement Account

description:
- This module will delete a vCenterManagement Account
options:
...
EXAMPLES = r'''
  - name: Delete a vCenter Management Account
    dellemc_vxrail_system_delete_management_accounts:
        vxmip: "{{ vxmip }}"
        vcadmin: "{{ vcadmin }}"
        vcpasswd: "{{ vcpasswd }}"
...
RETURN = r'''
Delete_Management_Accounts_Info:
  description: Delete a vCenter Management Account
  returned: always
  type: dict
  sample: >-
    {}
...
API = "DELETE /system/accounts/management"
MODULE = "dellemc_vxrail_system_delete_management_accounts"
LOG_FILE_PATH = "/tmp/vxrail_ansible_system_delete_management_accounts.log"
...
class VxRailCluster():
    ...
    def get_versioned_response(self, api_instance, module_path, account_info):
        ...
        call_string = self.api_version_string + '_system_accounts_management_delete'
        LOGGER.info("Using utility method: %s\n", call_string)
        system_accounts_management_delete = getattr(api_instance, call_string)
        return system_accounts_management_delete(body=account_info)

    def delete_management_account(self):
        account_info = {}
        account_info['component'] = self.component
        account_info['username'] = self.username
        ...
        try:
            ...
            LOGGER.error("Exception when calling ManagementAccountApi->%s_system_accounts_management_delete: %s\n", self.api_version_string, e)
            return 'error'
...
def main():
    ...
    result = VxRailCluster().delete_management_account()
    if result == 'error':
        module.fail_json(
            msg=f"Call {API} API failed, please see log file {LOG_FILE_PATH} for more error details.")
    vx_facts = {'Delete_Management_Accounts_Info': "success"}
    vx_facts_result = dict(changed=True, DELETE_MANAGEMENT_ACCOUNTS_API=vx_facts)
