...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_change_vcipaddress

short_description: Change the VxRail vCenter server IP address

description:
- This module will Change the VxRail vCenter server IP address
options:
  ...
  ip:
    description:
      The new IP address to assign to the vCenter Server
    required: False
    type: str

  gateway:
    description:
      The new gateway IP for the vCenter Server
    required: False
    type: str

  netmask:
    description:
      The new subnet mask for the vCenter Server
    required: False
    type: str

  ipv6:
    description:
      The new IPv6 address to assign to the vCenter Server
    required: False
    type: str

  gateway_ipv6:
    description:
      The new gateway IPv6 for the vCenter Server
    required: False
    type: str

  prefix_length_ipv6:
    description:
      The new prefix length for the vCenter Server
    required: False
    type: int
...
EXAMPLES = r'''
  - name: Change the VxRail vCenter server IP address
    dellemc_vxrail_change_vcipaddress:
        vxmip: "{{ vxmip }}"
        vcadmin: "{{ vcadmin }}"
        vcpasswd: "{{ vcpasswd }}"
        vcroot: "{{ vcroot }}"
...
LOGGER = utils.get_logger("dellemc_vxrail_system_vcipaddress_change.log", "/tmp/vxrail_ansible_system_vcipaddress_change.log", log_devel=logging.DEBUG)
...
class VxRailCluster():
    ...
    def get_versioned_response(self, api_instance, module_path, vcip_change_info):
        ...
        try:
          ...
          call_string = self.api_version_string + '_system_vcenter_ipaddress_' + 'post'
        except:
          call_string = "v1_system_vcenter_ipaddress_post"
          
        # Calls versioned method as attribute      
        LOGGER.info("Using utility method: %s\n", call_string)
        system_vcenter_ipaddress_post = getattr(api_instance, call_string)
        return system_vcenter_ipaddress_post(body=vcip_change_info)

    def post_system_vcenter_ipaddress(self):
        return_info = {}
        vcip_change_info = {
            "vc_info": {},
        ...
        if self.ip:
            vcip_change_info["vc_info"]["ip"] = self.ip
        if self.gateway:
            vcip_change_info["vc_info"]["gateway"] = self.gateway
        if self.netmask:
            vcip_change_info["vc_info"]["netmask"] = self.netmask
        if self.ipv6:
            vcip_change_info["vc_info"]["ipv6"] = self.ipv6
        if self.gateway_ipv6:
            vcip_change_info["vc_info"]["gateway_ipv6"] = self.gateway_ipv6
        if self.prefix_length_ipv6:
            vcip_change_info["vc_info"]["prefix_length_ipv6"] = self.prefix_length_ipv6

        LOGGER.info("Sending Configuration: %s", vcip_change_info)

        # create an instance of the API class
        api_instance = vxrail_ansible_utility.SystemInformationApi(
            vxrail_ansible_utility.ApiClient(self.configuration))
        try:
            response = self.get_versioned_response(api_instance, "POST /system/vcenter-ipaddress", vcip_change_info)
        except ApiException as e:
            LOGGER.error("Exception when calling SystemInformationApi->%s_system_vcenter_ipaddress_post: %s\n",
...
def main():
    ...
    result = VxRailCluster().post_system_vcenter_ipaddress()
    if result == 'error':
        module.fail_json(msg="Call POST system/vcenter-ipaddress API failed,"
                             "please see log file /tmp/vxrail_ansible_system_vcipaddress_change.log for more error details.")
    # vx_facts_result = dict(changed=True, change_VC_IPaddress=result)
    ...
    if result_response == 'error':
        module.fail_json(msg="Call v1/requests/request_id API failed,please see "
                             "log file /tmp/vxrail_ansible_system_vcipaddress_change.log for more error details.")
    else:
    ...
    if task_state == 'COMPLETED' and not error_message:
        LOGGER.info("change VC IPaddress Completed")
        output_msg = "change VC IPaddress has completed. Please see the logs at /tmp/vxrail_ansible_system_vcipaddress_change.log for more details"
        vx_facts = {'Request_ID': result_request_id, 'Request_Status': task_state}
        vx_facts_result = dict(changed=True, Change_VC_IPaddress=vx_facts, msg=task_state)
    else:
        LOGGER.info("change VC IPaddress Failed")
        output_msg = "Please see the /tmp/vxrail_ansible_system_vcipaddress_change.log for more details"
        vx_facts = {'Request_ID': result_request_id}
        vx_facts_result = dict(failed=True, Change_VC_IPaddress=vx_facts, msg=error_message)
