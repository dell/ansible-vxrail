...
DOCUMENTATION = r'''
---
module: dellemc_vxrail_system_internet_mode_change
...
LOGGER = utils.get_logger("dellemc_vxrail_system_internet_mode_change",
                          "/tmp/vxrail_ansible_system_internet_mode_change.log",
...
class VxRailCluster():
    ...
    def get_versioned_response(self, api_instance, module_path, internet_mode_info):
        ...
        # Calls versioned method as attribute (ex: v1_system_internet_mode_put)
        call_string = self.api_version_string + '_system_internet_mode_put'
        LOGGER.info("Using utility method: %s\n", call_string)
        system_internet_mode_put = getattr(api_instance, call_string)
        return system_internet_mode_put(internet_mode_info)

    def put_internet_mode(self):
        internet_mode_info = {}
        ...
        try:
            ...
            return 'error'
        LOGGER.info("%s/system/internet-mode PUT api response: %s\n", self.api_version_string, response)
...
def main():
    ...
    result = VxRailCluster().put_internet_mode()
    if result == 'error':
        module.fail_json(msg="Call PUT /system/internet-mode API failed,please see log file "
                             "/tmp/vxrail_ansible_system_internet_mode_change.log for more error details.")
    vx_facts = {'Internet_Mode_Change': result}
    vx_facts_result = dict(changed=True, System_Internet_Mode_API=vx_facts)
